...
void SVPWM_3ShuntInit(void)
{
    ...
    TIM1_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_CenterAligned1;//中央对齐模式1，输出比较中断标志位只在计数器向下计数时被设置    
    ...
    TIM1_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_Low;              //高电平有效
    ...
    TIM1_OCInitStructure.TIM_Pulse = PWM_PERIOD - 1; //比较值

    TIM1_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High;//有效状态high
    TIM1_OCInitStructure.TIM_OCNPolarity = TIM_OCNPolarity_Low;//disable
    TIM1_OCInitStructure.TIM_OCIdleState = TIM_OCIdleState_Reset;// 空闲状态low
    TIM1_OCInitStructure.TIM_OCNIdleState = TIM_OCNIdleState_Reset;//LOW_SIDE_IDLE_POLARITY;
    ...
    TIM1_BDTRInitStructure.TIM_Break = TIM_Break_Enable;//TIM_Break_Disable;
